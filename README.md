# 🎓 Airbnb Frontend role Assignment

## Project Description
This project is a small **React-based web application** .It allows users to explore multiple courses, each containing topics and subtopics rendered in **Markdown or text format**. The app focuses on:

- **Clean, responsive styling** using Tailwind CSS.
- **ARIA-friendly labels** for improved accessibility.
- Clear **heading hierarchy** in rendered Markdown content.
- Graceful **empty states** (e.g., “No topics yet”) when data is missing.
- Minimal admin surface to display a **read-only users list**.

The app demonstrates a professional approach to client-only React applications with good UX practices.

---

## ⚙️ Tech Stack

- **React + Vite** (JavaScript)
- **Tailwind CSS** for responsive and clean UI
- **Lucide React** for icons
- **Google Fonts (Nunito)** for typography
- **Vercel** for deployment
- Client-only app, with **localStorage** for persisting subtopic progress

---

## 🏗️ Setup Instructions

Follow these steps to set up the project locally:

1. **Clone the repository:**

git clone https://github.com/Rushi-Parekar11/Airbnb
cd your-repo

2. **Install dependencies:**

npm install


3. **Run the development server:""

npm run dev


4.**Build for production:**

npm run build


5.**Preview the production build:**

npm run preview
